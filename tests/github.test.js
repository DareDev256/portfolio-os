...
describe('GitHub.validateResponse()', () => {
        ...
        repos: [
            {
                ...
                forks_count: 1,
                language: 'JavaScript',
                html_url: 'https://github.com/DareDev256/repo-1',
                description: 'A repo',
            },
        ],
        events: [
    ...
    it('rejects missing user object', () => {
        expect(() => GitHub.validateResponse({ ...validData, user: null })).toThrow(
            'Invalid user data'
        );
    });

    it('rejects user without login string', () => {
        expect(() => GitHub.validateResponse({ ...validData, user: { login: 42 } })).toThrow(
            'Invalid user data'
        );
    });

    it('rejects non-array repos', () => {
        expect(() => GitHub.validateResponse({ ...validData, repos: 'not-array' })).toThrow(
            'Invalid repos data'
        );
    });

    it('rejects non-array events', () => {
        expect(() => GitHub.validateResponse({ ...validData, events: {} })).toThrow(
            'Invalid events data'
        );
    });
