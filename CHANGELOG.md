...
### The contact section now shows when he is actually free
A NEXT OPEN panel above the contact actions, listing real 30-minute afternoon
slots. Generated by `tools/build-availability.mjs`; booking still goes to
Calendly.

**Why not embed Calendly.** Inline embedding needs five CSP directives widened
and a third-party script with DOM access, on a page whose own argument is that
it ships no `unsafe-inline` in production. A hiring manager who reads response
headers would find the contradiction. So the page renders availability as its
own content — the same contract as every other figure here — and hands the
booking itself to a link.

**Why the car deadlines are the input.** Flat-rate work: a car marked `@ 2pm` is
a DELIVERY DEADLINE, not a block he sits in, so the schedule is his to sequence.
Measured over 164 cars across 120 days, the day's LAST deadline is the signal —
a call at 09:00 is clear of remaining work on 2% of days, at 16:00 on 74%, at
17:00 on 89%. Hence a 15:00–17:00 window, and slots only after the day's last
deadline.

### The bug that made this worth doing properly
The first generator modelled cars and nothing else, and offered **15:00 on
2026-09-02 — the exact half hour of the Exadel interview.** A booking page that
double-books its owner into a job interview is worse than no booking page.
`data/busy-blocks.json` now carries the real conflicts and is subtracted.

**That file holds times only, never titles.** It is committed and public, and
"Exadel Interview" on a public surface tells every other company where he is in
someone else's process. Cars are deliberately excluded from it — they are
deadlines, not occupancy, and counting them twice would close the afternoon for
nothing.

### Privacy is asserted, not assumed
The input is the dealership's data, not his. Computed availability may be
published; a stock number, customer or advisor may not. The generator refuses to
write if any of those reach the payload, and a test pins the same thing against
the committed file, so a change to either side fails.

### On the tests
Nine tests passed while the busy-block guard was deleted and the Exadel
collision was live. That is a suite that cannot see the failure it exists to
prevent, so a tenth was written specifically for it — mutation-checked, it is
the only one that fails when the guard goes.

The generator writes **nothing** when it cannot read its source: a booking page
guessing at availability is worse than none. So a failure renders as stale,
never as wrong, and past 30 hours the panel says how old it is. A failed fetch
hides the panel entirely rather than apologising — BOOK 30 MINUTES sits beside it
and already works.

Days beyond four out are labelled `unbooked-so-far` rather than "free": Maurice
books a median 4.1 days ahead, so an empty day past that horizon is unknown, not
open.
